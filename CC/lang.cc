...
int strings() {
    ...
    assert("hello" == "hello");
    assert("hello" < "Hello");
    assert("h" < "H");  // string order is different from ASCII char order
    assert('H' < 'h'); 
    std::string s1 = "hello";
    assert(s1.length()==5);
    assert(strlen(s1.c_str())==5); // strlen doesn't count terminating null
    assert(!s1.compare("hello"));
    assert(s1.compare("jello") < 0);
    //don't compare to -1 or 1; compare was returning -2
    assert(s1.compare("cello") > 0);
    /*
    char s2[6];
    strncpy(s2,s1,6);
...
int arrays() {
    ...
    int* ia=(int*) calloc(10,sizeof(int));  //I like calloc, inits the memory
    free(ia);
    ...
    ia=(int*) realloc(ia,20*sizeof(int));
    for (i=10; i<20; i++)
        ia[i]=-i;
    for (i=0; i<20; i++)
        printf("%d ",ia[i]);
